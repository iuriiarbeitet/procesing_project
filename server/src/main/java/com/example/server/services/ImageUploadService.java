...
import com.example.server.exception.ImageNotFoundException;
...
public class ImageUploadService {
    ...
    public void uploadImageToUser(MultipartFile file, Principal principal) throws IOException{
        ...
        imageModel.setImagesBytes(compressBytes(file.getBytes()));
    ...
    public void uploadImageToPost(MultipartFile file, Principal principal, Long postId) throws IOException{
        ...
        LOG.info("Uploading image to Pros {}", post.getId());
    ...
    public ImageModel getImageToPost(Long postId) {
        ImageModel imageModel = imageRepository.findByPostId(postId)
                .orElseThrow(() -> new ImageNotFoundException("Can not found image to Post: " + postId));
        if(!ObjectUtils.isEmpty(imageModel)){
            imageModel.setImagesBytes(decompressBytes(imageModel.getImagesBytes()));
        }
        return imageModel;
